...
function FalabellaView() {
  ...
  useEffect(() => {
    obtenerProductos("Falabella").then((data) => {
      setProductos(data);
      if (data.length > 0) {
  ...
  const getMainSpecs = (specs: any = {}) => {
    ...
    const modeloCpu = specs["Procesador específico txt"] || specs["Procesador"] || "";
    const cpu = marcaCpu && modeloCpu
    ...
    return {
      cpu: cpu.length > 25 ? cpu.substring(0, 25) + "…" : cpu,
      ram: specs["Memoria RAM"] || "N/A",
      ssd: specs["Capacidad de almacenamiento"] || "N/A",
      pantalla: specs["Tamaño de la pantalla"] ? `${specs["Tamaño de la pantalla"]}"` : "N/A",
    };
  };
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] p-4 sm:p-6 md:p-8 lg:p-12 animate-in fade-in duration-700">
      ...
      <header className="max-w-[1600px] mx-auto mb-8 md:mb-12 flex flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 bg-white p-1.5 pl-3 sm:pl-5 rounded-2xl sm:rounded-3xl border border-slate-200 shadow-sm">
          <div className="flex flex-col">
            <span className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Resultados</span>
      ...
      <div className="max-w-[1600px] mx-auto grid grid-cols-1 xl:grid-cols-5 gap-6 lg:gap-10">
        ...
        {/* SIDEBAR DE FILTROS ACCORDION EN MOBILE */}
        <aside className="xl:col-span-1">
          <div className="bg-white rounded-3xl xl:rounded-[40px] border border-slate-200/60 shadow-[0_8px_30px_rgb(0,0,0,0.02)] sticky top-6 overflow-hidden">
            ...
            {/* Botón Disparador en Mobile */}
            <div 
              className="flex items-center justify-between p-5 xl:p-8 cursor-pointer xl:cursor-default select-none border-b border-slate-100 xl:border-none"
            ...
            >
              <h3 className="text-base sm:text-lg font-black text-slate-900 flex items-center gap-3">
                <Filter size={18} className="text-[#00e5a0]" /> Filtros de búsqueda
              </h3>
            ...
            <div className={`p-6 xl:p-8 pt-2 xl:pt-0 space-y-8 ${mostrarFiltrosMobi ? 'block' : 'hidden xl:block'}`}>
              {/* Búsqueda */}
              <div>
                <label className="text-[10px] sm:text-[11px] font-bold uppercase text-slate-500 tracking-widest mb-3 block">Palabra Clave</label>
              ...
              <div>
                ...
                <div className="flex justify-between mt-2">
                  ...
                  <span className="text-xs font-bold text-slate-700">${filtroPrecio.toLocaleString("es-CO")}</span>
                </div>
              </div>
            </div>
        ...
        <div className="xl:col-span-4">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 md:py-32 bg-white rounded-3xl xl:rounded-[60px] border border-dashed border-slate-200 shadow-sm px-4 text-center">
              <div className="bg-slate-50 p-5 rounded-full mb-4">
                <Search className="text-slate-300" size={32} />
              </div>
              <p className="text-xl md:text-2xl font-black text-slate-800">Sin coincidencias</p>
              <p className="text-sm text-slate-400 mt-1 font-medium">Prueba modificando tus criterios de filtrado.</p>
            </div>
          ) : (
            <main className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
              {productosFiltrados.map((p) => {
                ...
                return (
                  ...
                  >
                    {/* Badge e Imagen */}
                    <div className="relative h-48 sm:h-56 p-6 sm:p-8 flex items-center justify-center bg-white">
                      <div className="absolute top-4 left-4 z-10 flex items-center gap-2">
                        <span className="bg-slate-900 text-white text-[9px] font-black px-3 py-1.5 rounded-xl shadow-md flex items-center gap-1.5 tracking-wider">
                      ...
                      <img
                        ...
                        onError={(e) => (e.currentTarget.src = "https://via.placeholder.com/400x300?text=Laptop")}
                      />
                    </div>
                    ...
                    <div className="p-5 sm:p-6 xl:p-8 pt-0 flex-1 flex flex-col">
                      ...
                      <div className="mt-auto pt-4 border-t border-slate-100 flex items-center justify-between">
                        <div>
                          <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Precio Hoy</p>
                        ...
                        <a
                          ...
                          className="h-11 w-11 sm:h-12 sm:w-12 flex items-center justify-center rounded-2xl bg-black text-[#00e5a0] border border-[#00e5a0]/40 hover:bg-[#00e5a0] hover:text-black transition-all shadow-md active:scale-90 flex-shrink-0"
                        >
...
// Subcomponente de Especificación adaptado
