...
function EbayView() {
  ...
  useEffect(() => {
    obtenerProductos("eBay").then((data) => {
      setProductos(data);
      if (data.length > 0) {
  ...
  const getMainSpecs = (specs: any = {}) => {
    const cpu = specs["Procesador"] || specs["Processor"] || specs["Modelo del procesador"] || "N/A";
    const ram = specs["Tamaño de RAM"] || specs["Memoria RAM"] || specs["RAM Size"] || "RAM N/A";
    const ssd = specs["Capacidad SSD"] || specs["Capacidad del disco duro"] || specs["SSD Capacity"] || "Storage N/A";
    const pantalla = specs["Tamaño de la pantalla"] || specs["Screen Size"] || "N/A";

    return {
      cpu: cpu.length > 25 ? cpu.substring(0, 25) + "…" : cpu,
      ram,
      ssd,
      pantalla: pantalla.length > 10 ? pantalla.substring(0, 10) : pantalla,
    };
  };
    ...
    <div className="min-h-screen bg-[#f8fafc] p-4 md:p-8 lg:p-12 animate-in fade-in duration-700">
      ...
      <header className="max-w-[1600px] mx-auto mb-12 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-1">
          <nav className="text-[10px] font-bold text-blue-600 uppercase tracking-[0.2em] mb-1">Global Marketplace Monitor</nav>
          <h1 className="text-5xl font-black text-slate-900 tracking-tight">
            eBay <span className="text-slate-300 font-light">Deals</span>
          </h1>
        </div>
        
        <div className="flex items-center gap-4 bg-white p-2 pl-5 rounded-3xl border border-slate-200 shadow-sm">
          <div className="flex flex-col">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Listados Activos</span>
            <p className="text-lg font-black text-slate-700 leading-none">{productosFiltrados.length}</p>
          </div>
          <div className="h-10 w-10 rounded-2xl bg-blue-50 flex items-center justify-center text-blue-600 font-bold">
            <Box size={20} />
          </div>
        </div>
      </header>
            ...
            <div className="space-y-10">
              ...
              <div>
                <div className="flex justify-between items-end mb-4">
                  <label className="text-[11px] font-bold uppercase text-slate-500 tracking-widest">Presupuesto (USD)</label>
                  <span className="text-xs font-black px-2 py-1 bg-blue-50 text-blue-600 rounded-lg">
                    ${filtroPrecio.toLocaleString()}
                  </span>
                </div>
                <input 
                  type="range"
                ...
                <div className="flex justify-between mt-3 text-[10px] font-bold">
                  <span className="text-slate-300">$0 USD</span>
                  <span className="text-slate-700">${maxPrecioOriginal.toLocaleString()} USD</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="xl:col-span-4">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-32 bg-white rounded-[60px] border border-dashed border-slate-200 shadow-sm">
              ...
              <p className="text-2xl font-black text-slate-800">Sin coincidencias</p>
              <p className="text-slate-400 mt-2 font-medium">Prueba ajustando el rango de precio o el término de búsqueda.</p>
            </div>
          ) : (
            <main className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {productosFiltrados.map((p) => {
                ...
                return (
                  <article
                    key={p._id}
                    className="group bg-white rounded-[40px] border border-slate-200/60 shadow-[0_10px_40px_rgba(0,0,0,0.03)] hover:shadow-[0_20px_60px_rgba(37,99,235,0.1)] transition-all duration-700 hover:-translate-y-2 flex flex-col overflow-hidden"
                  >
                    {/* Badge y Visual */}
                    <div className="relative h-60 p-10 flex items-center justify-center">
                      ...
                      <img
                        ...
                        onError={(e) => (e.currentTarget.src = "https://via.placeholder.com/400x300?text=eBay+Item")}
                      />
                    </div>
                    ...
                    <div className="px-8 pb-8 flex-1 flex flex-col">
                      ...
                      <div className="mt-auto pt-6 border-t border-slate-50 flex items-center justify-between">
                        <div>
                          <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-0.5">Precio hoy</p>
                          <p className="text-2xl font-black text-slate-900 tracking-tight">
                            {p.precio.toLocaleString("en-US", { style: "currency", currency: "USD" })}
                          </p>
                        </div>
                        <a
                          href={p.url}
